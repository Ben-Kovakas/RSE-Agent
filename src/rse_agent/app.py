...
if st.button("Generate & Validate"):
    if user_query:
        ...
        if previous_attempts:
            st.subheader("Code Iterations")
            if len(previous_attempts) == 1:
                st.info("ℹ️ Only 1 iteration: Tests passed on first attempt. The agent will retry if tests fail.")
            
            # Create tabs for each iteration with test results
            if len(previous_attempts) > 1:
                tabs = st.tabs([f"{i+1}{'st' if i == 0 else 'nd' if i == 1 else 'rd' if i == 2 else 'th'} iteration" 
                ...
                for idx, (tab, code_version) in enumerate(zip(tabs, previous_attempts)):
                    with tab:
                        ...
                        iter_num = idx + 1
                        result = next((r for r in executor_results if r["iteration"] == iter_num), None)
                        if result:
                            if result["passed"]:
